...
def run_part_plan_checks(full_plan: dict[str, Any], part_spec: dict[str, Any], code: str) -> list[str]:
    ...
    part_context = json_like_text({"full_plan": full_plan, "part_spec": part_spec})

    is_gear_part = any(
        marker in part_context.lower()
        for marker in ("gear", "spur_gear", "齿轮", "太阳轮", "行星轮", "内齿圈", "齿圈")
    )
    if is_gear_part:
        forbidden_low_level_calls = [
            "SelectByID2",
            "InsertSketch2",
            ".sw_doc",
            ".sw_instance",
        ...
        used_forbidden = [token for token in forbidden_low_level_calls if token in code]
        if used_forbidden:
            issues.append(
                "HARD: 齿轮标准件二次切除应使用封装流程 "
                "`sketch = sgear.insert_sketch_on_plane('XY'); sgear.create_circle(...); "
                "sgear.extrude_cut(sketch, depth)`，不要调用底层 "
                + ", ".join(used_forbidden)
                + "。"
            )
    ...
    if full_plan.get("request_type") == "assembly" and interface_names and not any(name in code for name in interface_names):
        issues.append("WARN: 装配零件代码中没有体现任何规划接口名称，可能无法支撑后续装配引用。")
...
def interface_item_name(item: Any) -> str:
