...
- 最终保存 `.SLDPRT`
- 装配任务中尽量暴露后续装配实际需要的接口名称；独立单零件任务不要为了接口而添加无用参考几何
...
因此：
- 代码中所有建模尺寸都应使用 **m**
- 若输入尺寸是 mm 或 cm，必须先换算为 m
- 不要把 10 mm 直接当作 10 m 传入 API
- `set_global_variable` 修改的是 SolidWorks 方程式，应沿用方程式原本单位写带单位字符串，例如 `"2mm"` 或 `"2cm"`；不要按建模 API 的米制规则一律改写成 m
- 参考轴端点也使用 m，且轴长度应接近零件尺寸，不能因为单位错用创建数米级长轴
...
- 装配接口：只有装配规划或用户明确要求会引用时，创建命名参考面 / 命名参考轴

## 6. 接口实现原则

如果零件 JSON 中要求接口，请先区分任务类型：独立 `part` 任务中的接口通常是可读性/复用建议；`assembly` 任务中被约束引用的接口才是必须实现项。需要实现接口时，优先这样实现：
...
5. 如果是装配零件或用户明确要求接口，创建参考面/参考轴接口
6. 保存到指定路径
7. 对失败点进行基本判空或异常处理
...
- 可复现
- 可重试
- 在装配任务中可被装配阶段继续引用接口的零件模型；独立零件任务以几何、尺寸和功能特征正确为主
