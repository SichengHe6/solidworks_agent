...
class AgentToolExecutor:
    def __init__(
        ...
        self.read_only_roots = [
            root.resolve()
            for root in (read_only_roots or [self.knowledge_base_root])
            if root is not None
        ]
    ...
    def write_file(self, path: str, content: str) -> ToolResult:
        file_path = self._resolve_writable_path(path, must_exist=False)
...
""",
    ...
    def _resolve_allowed_path(self, raw_path: str, must_exist: bool) -> Path:
        ...
        allowed_roots = [*self.workspace_roots, *self.read_only_roots]
    ...
    def _resolve_writable_path(self, raw_path: str, must_exist: bool) -> Path:
        resolved = self._resolve_allowed_path(raw_path, must_exist=must_exist)
        if any(self._is_relative_to(resolved, root) for root in self.read_only_roots):
            raise ValueError(f"Write path is read-only: {resolved}")
        if not any(self._is_relative_to(resolved, root) for root in self.workspace_roots):
            raise ValueError(f"Write path is outside writable workspace roots: {resolved}")
        return resolved
